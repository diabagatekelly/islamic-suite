...
class NoonSaakinRules():
  """Noon Saakin Rule Factory

  Find tajweed rules based on the silent letter "ن" (noon) or tanween ( ً ٍ ٌ). 
  ...
  **Notes: 
  ...
  def _find_tanween_base_in_text(self):
    ...
    indices_for_tanween_base = [
      ...
      if tanween in "ًٌٍ" and tanween_index < self._find_final_index()
      and not (tanween_index + 2 > len(self.ayah_text) - 1)
    ]
