...
class TestFileSystem(unittest.TestCase):
  ...
  def test_get_files_in_flat_directory(self):
    ...
    filenames = mock_file_system.get_files_in_directory(OUTPUTS_DIR)
    self.assertTrue(type(filenames), list)
    self.assertIsInstance(filenames[0], dict)
    self.assertTrue({'name': 'test.json', 'absolute_path': os.path.join(OUTPUTS_DIR, 'test.json')} in filenames)
