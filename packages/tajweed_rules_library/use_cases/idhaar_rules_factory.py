...
class IdhaarRulesFactory(SingleRuleFiles):
  ...
  def generate_idhaar_rule_file(self):
    ...
    for line in quran:
      ...
      noon_saakin_indices = self.find_all_letter_indices('ن', text)
      kasra_dummah_tanween_indices = self.find_all_tanween_indices('ٌٍ' , text)
      fatha_tanween_indices = self.find_all_tanween_indices('ً' , text)

      noon_idhaar = self.extract_letter_rule_data('idhaar', noon_saakin_indices, text, surah, ayah)
      if noon_idhaar:
        idhaar_locations.append(noon_idhaar)

      kasra_dummah_tanween_idhaar = self.extract_tanween_idhaar_rule_data('kasra', kasra_dummah_tanween_indices, text, surah, ayah)
      if kasra_dummah_tanween_idhaar:
        idhaar_locations.append(kasra_dummah_tanween_idhaar)

      fatha_tanween_idhaar = self.extract_tanween_idhaar_rule_data('fatha', fatha_tanween_indices, text, surah, ayah)
